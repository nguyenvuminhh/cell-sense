...
async def _create_chat(
    session: AsyncSession, user_id: int, chat: ChatRequest
) -> Chat:
    chat_data = ChatSchema(user_id=user_id, **chat.model_dump())
...
async def _get_chats_by_user(session: AsyncSession, user_id: int) -> list[Chat]:
    ...
    return [Chat(**chat.__dict__) for chat in chat_records]


async def _update_chat(
    session: AsyncSession, chat_id: int, chat_update: ChatRequest
) -> Chat | None:
    query = select(ChatSchema).where(ChatSchema.id == chat_id)
    result = await session.execute(query)
    chat_record = result.scalar_one_or_none()
    if chat_record:
        for key, value in chat_update.model_dump().items():
            setattr(chat_record, key, value)
...
async def create_chat(user_id: int, chat: ChatRequest) -> Chat:
    async with get_database_async_session() as session:
        async with session.begin():
            return await _create_chat(session, user_id, chat)
...
async def update_chat(chat_id: int, chat_update: ChatRequest) -> Chat | None:
    async with get_database_async_session() as session:
        async with session.begin():
            return await _update_chat(session, chat_id, chat_update)
